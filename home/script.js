...
/* createGame */
	...
	function createGame() {
		...
		sendPost(post, function(data) {
			if (!data.success) {
				document.getElementById("error").textContent = data.message || "unable to create game"
			}
			else {
...
/* joinGame */
	...
	function joinGame() {
		...
		if (gameCode.length !== 4) {
			document.getElementById("error").textContent = "game code must be 4 characters"
		}
		else if (!isNumLet(gameCode)) {
			document.getElementById("error").textContent = "game code must be letters and numbers only"
		}
		else {
			var post = {
				action: "joinGame",
			...
			sendPost(post, function(data) {
				if (!data.success) {
					document.getElementById("error").textContent = data.message || "unable to join game"
				}
				else {
					window.location = data.location
				}
